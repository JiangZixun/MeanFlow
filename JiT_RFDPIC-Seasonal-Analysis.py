...
class VideoLightningModule(pl.LightningModule):
    ...
    # Himawari-8/9 seasonal index mapping
    def get_seasonal_idx_list(self, season: str = None):
            if season == 'spring':
                return list(range(147, 365))  # 含147到364
            elif season == 'summer':
                return list(range(365, 577))  # 含365到576
            elif season == 'autumn':
                return list(range(577, 797))  # 含577到796
            elif season == 'winter':
                return list(range(797, 865)) + list(range(0, 147))  # 跨年：12月 + 1~2月
            else:
                return None
    ...
    def test_step(self, batch, batch_idx):
        # if batch_idx in self.get_seasonal_idx_list(self.args.season):
        if True:
