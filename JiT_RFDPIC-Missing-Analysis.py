...
def predict_next_frame_by_flow(frames: torch.Tensor):
    T, C, H, W = frames.shape
    prev = (frames[-2].mean(0).cpu().numpy() * 255).astype(np.uint8)
    last = (frames[-1].mean(0).cpu().numpy() * 255).astype(np.uint8)
    ...
    grid_t = torch.from_numpy(grid).float()[None]    # [1,H,W,2]
    last_frame = frames[-1][None]                   # [1,C,H,W]
    pred = F.grid_sample(last_frame, grid_t,
                        mode='bilinear',
                        padding_mode='border',
                        align_corners=True)
    return pred[0]  # [C,H,W]
...
class VideoLightningModule(pl.LightningModule):
    ...
    # for data missing analysis
    def _add_missing(self, data: torch.Tensor):
        def mask_missing_frames(inputs, missing_frames):
            masked = inputs.clone()
            # for t in missing_frames:
            masked[:, missing_frames] = 0.0
            return masked
        return mask_missing_frames(data, self.args.missing_frame)

    def _add_missing_copy(self, data: torch.Tensor):
        def fill_if_frame5_missing(inputs, missing_frames):
            filled = inputs.clone()
            if missing_frames == 5 and inputs.shape[1] > 5:
                filled[:, 5] = filled[:, 4]  # 用第4帧内容填充第5帧
            return filled
        return fill_if_frame5_missing(data, self.args.missing_frame)

    def _add_missing_optical_flow(self, data: torch.Tensor):
        def fill_if_frame5_missing(inputs, missing_frames):
            filled = inputs.clone()
            if missing_frames == 5:
                ...
                dtype  = inputs.dtype
                if inputs.ndim == 4:
                    # [T,C,H,W]：用第3、4帧作为输入，调用你现有的predict_next_frame_by_flow
                    assert inputs.shape[0] > 5, "sequence length must be > 5"
                    frames_34 = inputs[3:5].detach().cpu()                 # [2,C,H,W]
                    pred5 = predict_next_frame_by_flow(frames_34).to(device=device, dtype=dtype)  # [C,H,W]
                    filled[5] = pred5
                elif inputs.ndim == 5:
                    # [B,T,C,H,W]：对每个batch独立处理
                    B, T, C, H, W = inputs.shape
                    assert T > 5, "sequence length must be > 5"
                    for b in range(B):
                        frames_34 = inputs[b, 3:5].detach().cpu()          # [2,C,H,W]
                        pred5 = predict_next_frame_by_flow(frames_34).to(device=device, dtype=dtype)  # [C,H,W]
                        filled[b, 5] = pred5
                else:
                    raise ValueError("inputs must be [T,C,H,W] or [B,T,C,H,W]")
            return filled
        return fill_if_frame5_missing(data, self.args.missing_frame)
    ...
    def test_step(self, batch, batch_idx):
        if True:
            ...
            # inputs = self._add_missing(inputs)
            if self.args.missing_mode == 'normal':
                c_past = self._add_missing(c_past)
            elif self.args.missing_mode == 'copy' and self.args.missing_frame == 5:
                c_past = self._add_missing_copy(c_past)
            elif self.args.missing_mode == 'optical_flow' and self.args.missing_frame == 5:
                c_past = self._add_missing_optical_flow(c_past)
...
def main():
    ...
    # parser.add_argument('--season', type=str, default=None, choices=['spring', 'summer', 'autumn', 'winter'], help="Season filter for testing (optional)")
    parser.add_argument('--missing_frame', type=int, default=None, help="The missing frame index for data missing analysis (optional)")
    parser.add_argument('--missing_mode', type=str, default=None, choices=['normal', 'copy', 'optical_flow'], help="The missing frame handling mode (optional)")
