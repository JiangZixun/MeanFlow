...
class VideoLightningModule(pl.LightningModule):
    ...
    def _add_noise(self, data: torch.Tensor):
        ...
        def add_noise(inputs, noise_type='additive', noise_std=0.05):
            """
            在输入张量上添加噪声。
            
            参数:
            - inputs: Tensor, 形状为 (B, T, C, H, W)，归一化在 [0, 1]
            - noise_type: 'additive' 或 'multiplicative'
            - noise_std: 噪声的标准差 (float)
            
            返回:
            - noisy_inputs: Tensor, 添加噪声后的输入
            """
            if noise_std == 0 or noise_type == 'none':
                return inputs
        ...
        return add_noise(data, self.args.noise_type, self.args.noise_std)
...
def main():
    ...
    parser.add_argument('--noise_type', type=str, default='none', choices=['additive', 'multiplicative'], help="Type of noise to add to input context frames during testing")
    parser.add_argument('--noise_std', type=float, default=0.0, help="Standard deviation of noise to add to input context frames during testing")
