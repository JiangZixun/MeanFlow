...
from utils.lcs import (
    compute_lcs_weight_map,
    apply_lcs_conditioning,
    ridge_weighted_gradient_l1,
)
...
class VideoLightningModule(pl.LightningModule):
    ...
    def __init__(self, model_config, data_config, meanflow_config, optimizer_config, scheduler_config, training_config, logging_config, eval_config,
        ...
        # LCS settings (kept in meanflow config for minimal config changes)
        self.use_lcs = meanflow_config.get('lcs_enabled', True)
        self.lcs_mode = str(meanflow_config.get('lcs_mode', 'both')).lower()
        valid_lcs_mode = {'off', 'condition_only', 'loss_only', 'both'}
        if self.lcs_mode not in valid_lcs_mode:
        ...
        self.lcs_use_loss = self.use_lcs and self.lcs_mode in {'loss_only', 'both'}
    ...
    def build_lcs_condition(self, c_past: torch.Tensor, displacement_fields: torch.Tensor):
        """
        Build LCS-enhanced condition from RFDPIC displacement fields.
        Returns:
            c_cond: [B, T, C, H, W]
            lcs_weight: [B, T, 1, H, W]
            ridge_mask: [B, T, 1, H, W]
        """
        if not self.use_lcs:
            ones = torch.zeros_like(c_past[:, :, :1])
            return c_past, ones, ones
        ...
        if self.lcs_use_condition:
            c_cond = apply_lcs_conditioning(
                ...
                alpha=self.lcs_alpha,
                clamp_min=0.0,
                clamp_max=1.0,
            )
        else:
            c_cond = c_past
        return c_cond, lcs_weight, ridge_mask
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            c_rfdpic_norm, displacement_fields, _, _, _ = self.rfdpic_model(c_past_norm)
            c_rfdpic = inverse_data_transform(c_rfdpic_norm, rescaled=self.rfdpic_rescaled)
            c_rfdpic = c_rfdpic.detach() # (B, 6, C, H, W)
            c_cond, lcs_weight, ridge_mask = self.build_lcs_condition(c_past, displacement_fields)

        # --- 🔴 1. 修改：将 c 作为元组 (c_start, c_cond) 传递 ---
        # c_start = c_rfpic
        # c_cond = c_past
        loss_main, mse_val = self.meanflow.loss(self.model, x_future, c=(c_rfdpic, c_cond))

        # Auxiliary LCS ridge-weighted gradient loss (trainable)
        loss_lcs_grad = torch.tensor(0.0, device=self.device)
        if self.lcs_use_loss and self.lcs_lambda_grad > 0:
            x_target_norm = self.meanflow.normer.norm(x_future)
            z_init = self.meanflow.normer.norm(c_rfdpic)
            bsz = x_future.shape[0]
            t_aux = torch.ones((bsz,), device=self.device, dtype=x_future.dtype)
            r_aux = torch.zeros((bsz,), device=self.device, dtype=x_future.dtype)
            x_pred_aux = self.model(z_init, t_aux, r_aux, c_cond)
            loss_lcs_grad = ridge_weighted_gradient_l1(
                pred=x_pred_aux,
                target=x_target_norm,
                lcs_weight=lcs_weight,
                ridge_mask=ridge_mask,
                ridge_boost=self.lcs_ridge_boost,
            )

        loss = loss_main + self.lcs_lambda_grad * loss_lcs_grad
        
        self.log('train/loss', loss, on_step=True, on_epoch=False, prog_bar=True)
        self.log('train/mse_loss', mse_val, on_step=True, on_epoch=False, prog_bar=False)
        self.log('train/loss_main', loss_main, on_step=True, on_epoch=False, prog_bar=False)
        self.log('train/lcs_grad_loss', loss_lcs_grad, on_step=True, on_epoch=False, prog_bar=False)
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        if self.trainer.is_global_zero and self.global_step > 0 and self.global_step % save_freq == 0:
            ...
            with torch.no_grad():
                ...
                c_rfdpic_norm_val, displacement_fields_val, _, _, _ = self.rfdpic_model(c_past_norm_val)
                c_rfdpic_val = inverse_data_transform(c_rfdpic_norm_val, rescaled=self.rfdpic_rescaled)
                c_cond_val, _, _ = self.build_lcs_condition(c_past_val, displacement_fields_val)
                
                # c_start = c_rfdpic_val
                # c_cond = c_past_val
                c_tuple_val = (c_rfdpic_val, c_cond_val)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log('test/lcs_mean_batch', lcs_weight.mean(), prog_bar=False, on_step=True, on_epoch=False, sync_dist=False)
        self.log('test/lcs_ridge_ratio_batch', ridge_mask.mean(), prog_bar=False, on_step=True, on_epoch=False, sync_dist=False)
